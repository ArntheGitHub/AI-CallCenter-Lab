...
namespace ContosoSuitesWebAPI.Plugins
{
    ...
    public class MaintenanceRequestPlugin(CosmosClient cosmosClient)
    {
        ...
        public async Task<MaintenanceRequest> CreateMaintenanceRequest(Kernel kernel, int HotelId, string Hotel, 
            [Description("Defines who created the request, for example customer, maintenance, inspector, etc.")] string source,
            string Details, int? RoomNumber, string? location)
        {
            try
            {
